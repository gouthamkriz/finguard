...
const navigation: Array<{ id: Page; label: string; detail: string }> = [
  ...
  { id: "investigations", label: "Investigations", detail: "Evidence review" },
];
...
export function AppShell({ page, setPage, children, healthState, health, refreshHealth }: { page: Page; setPage: (page: Page) => void; children: ReactNode; healthState: "checking" | "connected" | "unavailable"; health: HealthResponse | null; refreshHealth: () => void }) {
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark">F</span><div><strong>FinGuard</strong><small>Graph-backed fraud investigation</small></div></div><nav aria-label="Primary navigation">{navigation.map((item) => <button type="button" key={item.id} className={page === item.id ? "nav-item active" : "nav-item"} onClick={() => setPage(item.id)}><strong>{item.label}</strong><small>{item.detail}</small></button>)}</nav><p className="sidebar-note">Trace suspicious relationships across customers, accounts, devices, IPs, and merchants using the live FinGuard API.</p></aside><main><header className="topbar"><div><p className="eyebrow">FINANCIAL CRIME INTELLIGENCE</p><h1>{navigation.find((item) => item.id === page)?.label}</h1></div><HealthIndicator state={healthState} health={health} onRefresh={refreshHealth} /></header><section className="page-content">{children}</section></main></div>;
}
